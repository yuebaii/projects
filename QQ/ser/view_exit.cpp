...
v
view_exit::view_exit(int cli_fd)
{
	cli_fds = cli_fd;
}

void view_exit::processs(Json::Value val,int cli_fd)
{
	//先尝试从缓存redis删除用户
	cout<<"talk_one"<<endl;
	  //初始化redis
	...
	Json::Value root;
	root["exitmessages"] = "您退出成功！";
	if(-1 == send(cli_fd,root.toStyledString().c_str(),strlen(root.toStyledString().c_str()),0))
	{
